...
public class MetricsServiceTests
{
    ...
    public void Compute_CalculatesTotalsAndStatusDistribution()
    {
        ...
        DashboardMetrics metrics = Service.Compute(store.Snapshot(), 3, latest, now: now);

        Assert.Equal(3, metrics.TotalRequests);
        Assert.Equal(3, metrics.StatusRequestCount);
        Assert.Equal(3, metrics.WindowRequests);
        Assert.Equal(0, metrics.LostAuditEntries);
        Assert.Equal(6, metrics.RequestsPerMin);
    ...
    {
        DateTimeOffset now = DateTimeOffset.UtcNow;
        DateTimeOffset recentTimestamp = now.AddSeconds(-5);
        ...
        DashboardMetrics metrics = Service.Compute(store.Snapshot(), 3, recent1, now: now);

        Assert.Equal(60, metrics.RequestsOverTime.Count);
    ...
    public void Compute_AvgResponseTimeExcludesEntriesOutsideWindow()
    {
        ...
        DashboardMetrics metrics = Service.Compute(store.Snapshot(), 2, recent, now: now);
    ...
    public void Compute_AvgTargetResponseTimeUsesTargetDurationsInsideWindow()
    {
        ...
        DashboardMetrics metrics = Service.Compute(store.Snapshot(), 4, first, now: now);
    ...
    public void Compute_UsesTimestampForLatestRequest()
    {
        ...
        DashboardMetrics metrics = Service.Compute(store.Snapshot(), 2, newest, now: now);

        Assert.Same(newest, metrics.LatestRequest);
    }

    [Fact]
    public void Compute_AnchorsRecentRateToCurrentTime()
    {
        DateTimeOffset now = DateTimeOffset.UtcNow;
        DateTimeOffset oldCapture = now.AddMinutes(-30);
        ...
        Assert.Equal(0, metrics.RequestsPerMin);
        Assert.Equal(60, metrics.RequestsOverTime.Count);
        Assert.Equal(0, metrics.RequestsOverTime.Sum(point => point.Value));
    }

    [Fact]
    public void Compute_DecaysRequestsPerMinWhileIdle()
    {
        DateTimeOffset requestTime = DateTimeOffset.UtcNow;
        MetricsStore store = BuildStore([Entry(requestTime)]);

        DashboardMetrics initial = Service.Compute(store.Snapshot(), 1, null, now: requestTime);
        DashboardMetrics idle = Service.Compute(store.Snapshot(), 1, null, now: requestTime.AddSeconds(29));

        Assert.Equal(60, initial.RequestsPerMin);
        Assert.Equal(2, idle.RequestsPerMin);
        Assert.Equal(60, idle.RequestsOverTime.Count);
        Assert.Equal(1, idle.RequestsOverTime.Sum(point => point.Value));
    }
    ...
    public void Compute_UsesAggregateSnapshotWhenRetainedEntriesAreTrimmed()
    {
        ...
        DashboardMetrics metrics = Service.Compute(store.Snapshot(), 1, latest, now: now);
    ...
    public void Compute_ProjectsRequestsPerMinFromObservedRate()
    {
        ...
        DashboardMetrics metrics = Service.Compute(store.Snapshot(), 2, null, now: now);
    ...
    public void Compute_IncludesAuditLossInAggregateMetrics()
    {
        ...
        DashboardMetrics metrics = Service.Compute(store.Snapshot(), 1, retained, now: now);
    ...
    [Fact]
    public void Compute_UsesRetainedStatusCountsWithoutChangingAggregateAverages()
    {
        DateTimeOffset now = DateTimeOffset.UtcNow;
        ...
        MetricsStore store = BuildStore([retained, deleted]);

        DashboardMetrics metrics = Service.Compute(
            store.Snapshot(),
            retainedRequestCount: 1,
            latestRequest: retained,
            retainedStatusCounts: new Dictionary<int, int> { [200] = 1 },
            now: now);

        Assert.Equal(1, metrics.TotalRequests);
        Assert.Equal(1, metrics.StatusRequestCount);
        Assert.Equal(2, metrics.WindowRequests);
        Assert.Equal(200, metrics.AvgResponseTime);
        Assert.Equal(["200"], metrics.StatusDistribution.Select(group => group.Label));
    }
